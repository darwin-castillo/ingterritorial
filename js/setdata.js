...
function multiplePromise() {
    ...
    annio = !isEmpty(annio) ? annio : "2019";
    var values = [
...
window.onload = function () {
    ...
    console.log("Current year ", current_year, current_year - 1);
    ...
    if (findGetParameter('filter') === null && findGetParameter("status") === null && findGetParameter("a") == null) {
        ...
        var current_year = new Date().getFullYear();
        document.location.href = "?" + "status=1,2&a=" + current_year.toString() + "," + (current_year - 1).toString();
...
//  +++++++++++++++++++++++++++ departamento
        ...
        if (!vct_entidad.includes(val.nombre_de_la_entidad)) {
            ...
            option.setAttribute("value", valueOpt);
...
function multipleFetchApi() {
    ...
    var annio = findGetParameter('a');
    var current_year = new Date().getFullYear();
    annio = !isEmpty(annio) ? annio : current_year + "," + current_year - 1;
